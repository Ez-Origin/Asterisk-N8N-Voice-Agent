...
class LocalAIServer:
    def __init__(self):
        ...
        self.tts_model: Optional[PiperTTS] = None
    ...
    async def initialize_models(self):
        ...
        tts_model_path = "/app/models/tts/en_US-lessac-medium.onnx"
        ...
        if not os.path.exists(tts_model_path):
            raise FileNotFoundError(f"TTS model not found at {tts_model_path}")

        self.stt_model = VoskModel(stt_model_path)
        self.llm_model = Llama(model_path=llm_model_path, n_ctx=2048)
        self.tts_model = PiperTTS(tts_model_path)
        logging.info("All models loaded successfully - using high-quality Piper TTS")
    ...
    async def process_tts(self, text: str) -> bytes:
        # High-quality TTS processing using Piper TTS
        if self.tts_model:
            ulaw_bytes = self.tts_model.tts(text)
            return ulaw_bytes
        return b""
...
async def main():
    ...
    async with serve(server.handler, "0.0.0.0", 8765):
        logging.info("Local AI Server started on ws://0.0.0.0:8765 with Piper TTS")
