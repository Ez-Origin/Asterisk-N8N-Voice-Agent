#!/usr/bin/env python3
"""
Test Silero VAD + Whisper as a local STT solution for telephony
This combines excellent VAD for telephony with Whisper for accurate STT
"""
...
def test_silero_vad_whisper(audio_data: bytes, segment_info: dict) -> dict:
    """Test Silero VAD + Whisper locally"""
    try:
        # Convert to WAV
        ...
        try:
            # Test with Silero VAD + Whisper
            # First, try to use whisper command if available
            result = subprocess.run([
                'whisper',  # Assuming whisper is installed
                '--model', 'base',
                '--language', 'en',
                '--output-format', 'txt',
                '--vad_filter', 'True',  # Enable VAD filtering
                '--vad_threshold', '0.35',  # VAD threshold for telephony
                temp_file_path
...
def test_whisper_only(audio_data: bytes, segment_info: dict) -> dict:
    """Test Whisper without VAD for comparison"""
    try:
        # Convert to WAV
        wav_data = convert_raw_to_wav(audio_data, 16000)
...
async def compare_stt_providers(capture_dir: str):
    """Compare Vosk vs Silero VAD + Whisper vs Whisper only"""
    logging.info("🎯 SILERO VAD + WHISPER TESTING")
    logging.info("=" * 60)
    logging.info("Comparing Vosk vs Silero VAD + Whisper vs Whisper only")
    ...
    # Test each segment with all providers
    all_results = []
    ...
    for i, segment_info in enumerate(segments):
        ...
        # Test with Whisper only (local)
        logging.info("  Testing with Whisper only (local)...")
        whisper_result = test_whisper_only(combined_audio, segment_info)
        all_results.append(whisper_result)
        
        # Small delay between tests
        await asyncio.sleep(1.0)
        
        # Test with Silero VAD + Whisper (local)
        logging.info("  Testing with Silero VAD + Whisper (local)...")
        silero_result = test_silero_vad_whisper(combined_audio, segment_info)
        all_results.append(silero_result)
    ...
    whisper_success = sum(1 for r in all_results if r["provider"] == "whisper_only" and r["status"] == "success")
    silero_success = sum(1 for r in all_results if r["provider"] == "silero_vad_whisper" and r["status"] == "success")
    
    logging.info(f"\n🏆 OVERALL COMPARISON:")
    logging.info(f"  Vosk success rate: {vosk_success}/{len(segments)} ({vosk_success/len(segments)*100:.1f}%)")
    logging.info(f"  Whisper only success rate: {whisper_success}/{len(segments)} ({whisper_success/len(segments)*100:.1f}%)")
    logging.info(f"  Silero VAD + Whisper success rate: {silero_success}/{len(segments)} ({silero_success/len(segments)*100:.1f}%)")
    ...
    logging.info(f"\n🏆 BEST PROVIDER: {best_provider.upper()}")
